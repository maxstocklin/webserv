...
/*   Updated: 2023/09/20 20:27:49 by max              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	WebServer::readRequest(const int &i, Client &c)
{
	...
	if (c.request.get_handler_response().statusCode != 0 || requestCompletelyReceived(c.completeData) == true)
	{
		...
		c.buildResponse();													// todo


		if (c.response.getCgiState())										// todo
		{
			handleReqBody(c);												// todo
			addToSet(c.response._cgi_obj.pipe_in[1],  _write_fd_pool);		// todo
			addToSet(c.response._cgi_obj.pipe_out[0],  _recv_fd_pool);		// todo
		}
...
void    ServerManager::assignServer(Client &c)
{
	...
	for (std::vector<ListeningSocket>::iterator it = sockets.begin();
		it != sockets.end(); ++it)
	{
